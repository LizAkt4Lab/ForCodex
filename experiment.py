    ...
    def forward_user(self, batch: Dict, device: torch.device) -> Dict[str, torch.Tensor]:
        ...
        for time_step in batch["clicks"]:
            ...
            for doc in time_step["docs"]:
                ...
                total_words += float(bow.sum().item())
                total_docs += 1
                alpha_mu_norm += float(mu_alpha.pow(2).mean().detach().item())
                alpha_logvar_mean += float(logvar_alpha.detach().mean().item())
        ...
        return {
            ...
            "state_steps": total_state_steps,
            "alpha_mu_norm": alpha_mu_norm,
            "alpha_logvar_mean": alpha_logvar_mean,
        }
...
def train_model(data: Dict, device: torch.device, epochs: int = 30) -> Tuple[DynamicTopicModel, List[Dict[str, float]]]:
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in dataloader:
            ...
            epoch_words += outputs["word_count"]
            epoch_docs += outputs["doc_count"]
            epoch_state_steps += outputs["state_steps"]
            epoch_alpha_mu += outputs["alpha_mu_norm"]
            epoch_alpha_logvar += outputs["alpha_logvar_mean"]

        avg_loss = epoch_loss / len(dataset)
        avg_recon = epoch_recon / len(dataset)
        avg_kl_alpha = epoch_kl_alpha / len(dataset)
        avg_kl_s = epoch_kl_s / len(dataset)
        avg_recon_per_token = epoch_recon / max(epoch_words, 1.0)
        avg_kl_alpha_per_doc = epoch_kl_alpha / max(epoch_docs, 1)
        avg_kl_s_per_step = epoch_kl_s / max(epoch_state_steps, 1)
        avg_alpha_mu = epoch_alpha_mu / max(epoch_docs, 1)
        avg_alpha_logvar = epoch_alpha_logvar / max(epoch_docs, 1)
        history.append(
            {
                "epoch": epoch,
                "loss": avg_loss,
                ...
                "kl_s": avg_kl_s,
                "recon_per_token": avg_recon_per_token,
                "kl_alpha_per_doc": avg_kl_alpha_per_doc,
                "kl_s_per_step": avg_kl_s_per_step,
                "alpha_mu_norm": avg_alpha_mu,
                "alpha_logvar_mean": avg_alpha_logvar,
            }
        )
        print(
            "Epoch {epoch:02d} | Loss: {avg_loss:.2f} | Recon: {avg_recon:.2f} "
            "| KL_alpha: {avg_kl_alpha:.2f} | KL_s: {avg_kl_s:.2f} "
            "| Recon/token: {avg_recon_per_token:.4f} | KL_alpha/doc: {avg_kl_alpha_per_doc:.4f} "
            "| KL_s/step: {avg_kl_s_per_step:.4f} | ||mu_alpha||^2: {avg_alpha_mu:.4f} | mean logvar: {avg_alpha_logvar:.4f}".format(
                epoch=epoch,
                avg_loss=avg_loss,
                avg_recon=avg_recon,
                avg_kl_alpha=avg_kl_alpha,
                avg_kl_s=avg_kl_s,
                avg_recon_per_token=avg_recon_per_token,
                avg_kl_alpha_per_doc=avg_kl_alpha_per_doc,
                avg_kl_s_per_step=avg_kl_s_per_step,
                avg_alpha_mu=avg_alpha_mu,
                avg_alpha_logvar=avg_alpha_logvar,
            )
        )
